...
def _post_patched_request(consumers, lti_key, body,
    ...
    # not sure we do anything with this - does it protect from anything?
    # we had to disable utf-8 encoding in the xml encoding because it was
    # attempting to send byte strings rather than normal strings so we aren't
    # encoding unicode characters at the moment
    # sell also: oauth2 send_request - have to encode before hashing
...
def post_message(consumers, lti_key, url, body):
    ...
    #have to encode for comparison
    is_success = "<imsx_codeMajor>success</imsx_codeMajor>".encode('utf-8') in content
...
def generate_request_xml(message_identifier_id, operation,
    ...
    #added unicode encoding to avoid returning byte strings
